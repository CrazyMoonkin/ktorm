...
class MultipleLoggerTest : BaseTest() {

    private val traceLogger = TestLogger(threshold = LogLevel.TRACE)
    private val infoLogger = TestLogger(threshold = LogLevel.INFO)

    override fun init() {
        database = Database.connect(
            url = "jdbc:h2:mem:ktorm;DB_CLOSE_DELAY=-1",
            driver = "org.h2.Driver",
            loggers = listOf(traceLogger, infoLogger),
    ...
    @Test
    fun testLoggingWithMultipleLoggers() {

        database.from(Departments).select().rowSet

        infoLogger.messages[LogLevel.INFO]!!.let { infoLogList ->
            assert(infoLogList.size == 1)
            assert(
                infoLogList[0].first.startsWith("Connected to jdbc:h2:mem:ktorm, productName: H2, productVersion")
            )
        }
        assert(infoLogger.messages[LogLevel.TRACE]?.size == 0)
        assert(infoLogger.messages[LogLevel.DEBUG]?.size == 0)
        assert(infoLogger.messages[LogLevel.WARN]?.size == 0)
        assert(infoLogger.messages[LogLevel.ERROR]?.size == 0)

        traceLogger.messages[LogLevel.INFO]!!.let { infoLogList ->
            assert(infoLogList.size == 1)
            assert(
                infoLogList[0].first.startsWith("Connected to jdbc:h2:mem:ktorm, productName: H2, productVersion")
            )
        ...
        traceLogger.messages[LogLevel.DEBUG]!!.let { debugLogList ->
        ...
        assert(traceLogger.messages[LogLevel.TRACE]?.size == 0)
        assert(traceLogger.messages[LogLevel.WARN]?.size == 0)
        assert(traceLogger.messages[LogLevel.ERROR]?.size == 0)
    }
